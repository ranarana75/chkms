...
import { Bell, Megaphone, Users, AlertTriangle, FileText, Pin, Eye, Edit, Trash2, Plus, Calendar } from 'lucide-react';
    ...
    <div className="min-h-screen bg-gray-50">
                ...
                <div className="flex items-center space-x-4">
                  ...
                  <label className="flex items-center space-x-2">
                    <input
                      ...
                      onChange={(e) => setNewAnnouncement({ ...newAnnouncement, autoExpire: e.target.checked })}
                    />
                    <span>অটো মেয়াদ শেষ</span>
                  </label>
                </div>
                <Button onClick={handleCreateAnnouncement} className="w-full bg-orange-600 hover:bg-orange-700">
                  ঘোষণা প্রকাশ করুন
                </Button>
              </div>
            </DialogContent>
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{dashboardData.stats.activeNotices}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                <p className="text-sm font-medium text-gray-600">জরুরি নোটিশ</p>
                <p className="text-2xl font-bold text-gray-900">{dashboardData.stats.urgentNotices}</p>
              </div>
            </div>
          </CardContent>
        </Card>
          ...
          <CardContent className="p-4">
            <div className="flex items-center space-x-2">
              ...
              <div>
                ...
                <p className="text-2xl font-bold text-gray-900">{dashboardData.stats.totalAnnouncements}</p>
              </div>
            </div>
          </CardContent>
        </Card>
              ...
              {dashboardData.recentNotices.map((notice) => (
                <div key={notice.id} className="border rounded-lg p-4">
                  ...
                  <div className="flex justify-between items-center text-xs text-gray-500">
                    <span>লক্ষ্য: {getAudienceText(notice.targetAudience)}</span>
                    <span>{new Date(notice.publishDate).toLocaleDateString('bn-BD')}</span>
                  </div>
          ...
          <CardContent>
            <div className="space-y-4">
              {dashboardData.urgentAnnouncements.map((announcement) => (
                <div key={announcement.id} className="border rounded-lg p-4 bg-red-50">
                  <div className="flex justify-between items-start mb-2">
                  ...
                  <div className="flex justify-between items-center text-xs text-gray-500">
                    ...
                    <span>{new Date(announcement.publishDate).toLocaleDateString('bn-BD')}</span>
                  </div>
                  <div className="flex space-x-2 mt-3">
                    <Button 
                      size="sm" 
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>সকল নোটিশের তালিকা এবং বিস্তারিত তথ্য</CardDescription>
        </CardHeader>
        <CardContent>
              ...
              <tbody>
                {notices.map((notice) => (
                  <tr key={notice.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-2">{getAudienceText(notice.targetAudience)}</td>
                    <td className="p-2">
                      <Badge className={getPriorityColor(notice.priority)}>
                        {notice.priority === 'high' ? 'জরুরি' : notice.priority === 'medium' ? 'মধ্যম' : 'সাধারণ'}
                      </Badge>
                    </td>
                    <td className="p-2">{new Date(notice.publishDate).toLocaleDateString('bn-BD')}</td>
                    <td className="p-2">{notice.expiryDate ? new Date(notice.expiryDate).toLocaleDateString('bn-BD') : 'অসীম'}</td>
                    <td className="p-2">
                      <div className="flex space-x-2">
                        <Button size="sm" variant="outline">
