...
export default function UserProfile() {
  const { user, session, logout, changePassword, updateProfile, sessionExpiry } = useAuth();
  ...
  const handleChangePassword = async () => {
    ...
    try {
      ...
      if (result.success) {
        addNotification(result.message || 'পাসওয়ার্ড পরিবর্তিত হয়েছে', 'success');
        setShowPasswordDialog(false);
        setPasswordForm({ currentPassword: '', newPassword: '', confirmPassword: '' });
      } else {
        addNotification(result.message || 'পাসওয়ার্ড পরিবর্তনে ��মস্যা', 'error');
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-teal-100" style={{ fontFamily: '"Noto Serif Bengali", serif' }}>
      ...
      <div className="container mx-auto px-4 py-8">
        <div className="mb-8">
          <h1 className="text-4xl font-bold text-emerald-800 mb-2">ব্যবহারকারী প্রোফাইল</h1>
                ...
                <div className="space-y-3 text-sm">
                  ...
                  {user.lastLogin && (
                    <div className="flex items-center justify-center space-x-2">
                      ...
                      <span>শেষ লগইন: {new Date(user.lastLogin).toLocaleString('bn-BD')}</span>
                    </div>
                  )}
                </div>
            ...
            {session && (
              <Card className="mt-6">
                ...
                <CardContent className="space-y-3 text-sm">
                  <div>
                    <span className="font-medium">তৈরি:</span> {new Date(session.createdAt).toLocaleString('bn-BD')}
                  </div>
                  <div>
                    <span className="font-medium">শেষ সক্রিয়:</span> {new Date(session.lastActivity).toLocaleString('bn-BD')}
                  </div>
                  <div>
                    <span className="font-medium">মেয়াদ শেষ:</span> {sessionExpiry ? new Date(sessionExpiry).toLocaleString('bn-BD') : 'N/A'}
                  </div>
                  <div>
                    <span className="font-medium">ডিভাইস:</span> {session.deviceInfo}
                  </div>
                  ...
                  <CardHeader>
                    <div className="flex items-center justify-between">
                      <div>
                        ...
                        <CardDescription>আপনার প্রোফাইল তথ্য আপডেট করুন</CardDescription>
                      </div>
                      <Button
              ...
              <TabsContent value="security" className="space-y-6">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>আপনার অ্যাকাউন্ট নিরাপত্তা পরিচালনা করুন</CardDescription>
                  </CardHeader>
              ...
              <TabsContent value="permissions" className="space-y-6">
                <Card>
                  <CardHeader>
                    ...
                    <CardDescription>আপনার অ্যাকাউন্টের অনুমতি দেখুন</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <div className="space-y-4">
                      <div>
