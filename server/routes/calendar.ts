...
router.get("/dashboard", (req, res) => {
  ...
  const upcomingHolidays = holidays
    .filter((holiday) => new Date(holiday.startDate) > now && holiday.isActive)
    .slice(0, 3);
...
router.get("/events", (req, res) => {
  ...
  if (month && year) {
    filteredEvents = filteredEvents.filter((event) => {
      const eventDate = new Date(event.startDate);
      return (
        eventDate.getMonth() === parseInt(month as string) - 1 &&
...
router.get("/holidays", (req, res) => {
  ...
  if (active !== undefined) {
    filteredHolidays = filteredHolidays.filter(
      (holiday) => holiday.isActive === (active === "true"),
    );
  }
...
router.get("/date/:date", (req, res) => {
  ...
  const dayHolidays = holidays.filter((holiday) => {
    ...
    return (
      targetDate >= holidayStart && targetDate <= holidayEnd && holiday.isActive
    );
  });
