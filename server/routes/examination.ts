...
router.get("/dashboard", (req, res) => {
  ...
  const passedStudents = examResults.filter(
    (result) => result.status === "pass",
  ).length;
  const averageMarks =
    examResults.reduce((sum, result) => sum + result.obtainedMarks, 0) /
    examResults.length;
...
router.get("/examinations/:id/schedules", (req, res) => {
  ...
  const schedules = examSchedules.filter(
    (schedule) => schedule.examinationId === id,
  );
  res.json(schedules);
});
...
// Get report card
router.get(
  "/examinations/:examId/students/:studentId/report-card",
  (req, res) => {
    const { examId, studentId } = req.params;
    const reportCard = reportCards.find(
      (card) => card.examinationId === examId && card.studentId === studentId,
    );
...
router.post(
  ...
  (req, res) => {
    ...
    const cgpa =
      studentResults.reduce((sum, result) => sum + result.gpa, 0) /
...
router.get("/examinations/:id/statistics", (req, res) => {
  ...
  const filteredResults = examResults.filter(
    (result) => result.examinationId === id,
  );
  ...
  const highestMarks = Math.max(
    ...examResults.map((result) => result.obtainedMarks),
  );
  const lowestMarks = Math.min(
    ...examResults.map((result) => result.obtainedMarks),
  );

  const gradeDistribution = examResults.reduce(
    (acc, result) => {
